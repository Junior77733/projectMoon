...
const API_URL = 'https://api.atlasacademy.io/export/JP/nice_bgm_lang_en.json'
const OUTPUT_FILE = path.resolve(__dirname, 'src', 'other-bgm.json')
...
async function fetchServantSkillsJSON() {
    ...
    const formattedData = data.map(bgm => ({
            id: bgm.id,
            name: bgm.name
    }));
